...
class AttendanceBrain:
    ...
    def get_semester_end_date(self, start_date):
        ...
        if 1 <= month <= 6:
                ...
                year, self.sem_config["even_end"][0], self.sem_config["even_end"][1]
            )
            return start_date + timedelta(days=180) if start_date > end else end
        else:
            end = date(
                year, self.sem_config["odd_end"][0], self.sem_config["odd_end"][1]
            )
    ...
    def parse_timetable(self, file_path):
        ...
        try:
            ...
            if not col_to_time:
                return False, "Could not detect time slots."
                ...
                if first_col in days_map:
                    ...
                    for col_idx, time_str in col_to_time.items():
                        if col_idx < len(row):
                            ...
                            if len(raw_subject) > 2 and not any(
                                k == raw_subject.lower() for k in bad
                            ):
                                # CLEAN THE NAME HERE
            ...
            return (
                True,
                f"Parsed {sum(len(v) for v in self.timetable.values())} classes.",
            )
    ...
    def load_absence_details(self, file_path):
        try:
            ...
            abs_df["Date"] = pd.to_datetime(
                abs_df["Date"], dayfirst=True, errors="coerce"
            ).dt.date
            ...
            time_cols = [
                ...
                if "P" in c and c not in ["Total", "Percentage"]
            ]

            for idx, row in abs_df.iterrows():
                d = row["Date"]
                if self.is_holiday_or_off(d):
                    continue
                ...
                self.full_history.append(
                    {"Date": d, "Subject": "Daily_Aggregate", "IsAbsent": is_absent_day}
                )
    ...
    def train_models(self):
        ...
        X = [
            [r["Date"].weekday(), r["Date"].day, r["Date"].month]
    ...
    def predict_day_risk(self, date_obj):
        if not self.is_trained or self.is_holiday_or_off(date_obj):
            return 0.0
        try:
            return self.model_daily.predict_proba(
                [[date_obj.weekday(), date_obj.day, date_obj.month]]
            )[0][1]
        except:
            return 0.0
    ...
    def get_subject_risks(self):
        ...
        subjects = [
            x["Subject"] for x in self.full_history if x["Subject"] != "Daily_Aggregate"
        ]
        if not subjects:
            return []
    ...
    def calculate_recovery_plan(
        self, target_percent, manual_total, manual_absent, start_date, limit_date
    ):
        if manual_total < 0:
            manual_total = 0
        if manual_absent < 0:
                    ...
                    for time_str in sorted_times:
                        if classes_count < classes_needed:
                            schedule.append(
                                {
                                    "Date": sim_date,
                                    "Day": sim_date.strftime("%A"),
                                    "Time": time_str,
                                    "Subject": daily_classes[time_str],
                                }
                            )
                            classes_count += 1
